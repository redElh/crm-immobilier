...
import type { TransactionRole } from '../../../types/transactions'
...
export const ClientTransactionsTab = ({ clientId, clientName, clientType }: {
  clientId: string
  clientName: string
  clientType: string
}) => {
  const transactions = useMemo(() => getClientTransactions(clientId), [clientId])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-text-secondary">
          {transactions.length} transaction{transactions.length !== 1 ? 's' : ''} liée{transactions.length !== 1 ? 's' : ''} à <span className="font-medium text-text">{clientName}</span>
        </p>
      </div>
            ...
            {transactions.map((t, i) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <span className={`inline-block px-2 py-0.5 text-[10px] font-medium rounded-lg border ${roleColor(t.role)}`}>
                    {t.role}
                  </span>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-xs text-text-secondary">
                  {new Date(t.dateContracted).toLocaleDateString('fr-FR')}
                </td>
