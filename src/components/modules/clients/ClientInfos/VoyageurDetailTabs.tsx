...
import {
  ...
  Mail, RefreshCw, Link, Award, TrendingUp, Users, Moon, Sun as SunIcon
} from 'react-feather';
...
const TABS = [
  ...
  { id: 5, label: 'Paiement & Séjour' },
  { id: 6, label: 'Contrat' },
  { id: 7, label: 'Documents' },
  { id: 8, label: 'Transactions' },
  { id: 9, label: 'Contrats' },
  { id: 10, label: 'Notes & Activité' },
  { id: 11, label: 'Messages' },
];

export const VoyageurDetailTabs = ({ client: initialClient }: { client: Client }) => {
  ...
  const renderCriteres = () => {
    const hasData = client.propertyType || client.secteur || client.area || client.prixMin ||
      client.prixMax || client.pieces || client.chambres || client.nbPersonnes;
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InfoField label="Catégorie" value={client.categorie || 'Location saisonnière'} icon={<Tag size={16} className="text-accent" />} />
          ...
          {(client.prixMin || client.prixMax) && (
            <InfoField label="Budget par nuit" value={`${client.prixMin?.toLocaleString() || '?'} ~ ${client.prixMax?.toLocaleString() || '?'} ${devise}`} icon={<DollarSign size={16} className="text-accent" />} />
          )}
          {client.nbPersonnes && (
            <InfoField label="Couchages max" value={`${client.nbPersonnes} personnes`} icon={<Users size={16} className="text-accent" />} />
          )}
    ...
    return (
      <div className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {client.disponibilite && <InfoField label="Flexibilité" value={client.disponibilite} icon={<Clock size={16} className="text-accent" />} />}
        </div>
  ...
  const renderPaiementSejour = () => {
    const hasData = client.source || client.notes;
    ...
    return (
      <div className="space-y-5">
        {client.source && (
          <InfoField label="Origine de la réservation" value={client.source} icon={<MapPin size={16} className="text-accent" />} />
        )}
        {client.notes && (
          <div className="p-4 rounded-xl bg-background border border-border/50">
            <p className="text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">Informations sur le séjour</p>
            <p className="text-sm text-text/80">{client.notes}</p>
          </div>
        )}
      </div>
    );
  ...
  const renderContrat = () => {
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InfoField label="Numéro de réservation" value={client.numeroMandat} icon={<Tag size={16} className="text-accent" />} />
          {client.statutMandat && <InfoField label="Statut" value={client.statutMandat} icon={<Clock size={16} className="text-accent" />} />}
          {client.typeMandat && <InfoField label="Conditions d'annulation" value={client.typeMandat} icon={<Compass size={16} className="text-accent" />} />}
          {client.dateSignature && <InfoField label="Date de réservation" value={new Date(client.dateSignature).toLocaleDateString('fr-FR')} icon={<Calendar size={16} className="text-accent" />} />}
          {client.dateDebut && <InfoField label="Date d'arrivée" value={new Date(client.dateDebut).toLocaleDateString('fr-FR')} icon={<Calendar size={16} className="text-accent" />} />}
          {client.dateExpiration && <InfoField label="Date de départ" value={new Date(client.dateExpiration).toLocaleDateString('fr-FR')} icon={<Calendar size={16} className="text-accent" />} />}
          {client.conjoint && <InfoField label="Langues parlées" value={client.conjoint} icon={<Users size={16} className="text-accent" />} />}
          {client.dureeProtection && <InfoField label="Flexibilité" value={client.dureeProtection} icon={<Clock size={16} className="text-accent" />} />}
        </div>
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 5: return renderPaiementSejour();
      case 6: return renderContrat();
      case 7: return renderDocuments();
      case 8: return renderTransactions();
      case 9: return renderContrats();
      case 10: return renderNotesActivite();
      case 11: return renderMessages();
      default: return null;
    }
  };
