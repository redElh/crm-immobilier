  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {items.map(item => (
          <span key={item} className={isGerant ? "px-2 py-1 text-xs rounded-lg bg-[#E7D5D5] text-[#905D5D] border border-[#E0C6C6]" : "px-2 py-1 text-xs rounded-lg bg-accent/10 text-accent border border-accent/20"}>
            {item}
          </span>
        ))}
      </div>
...
const renderProximites = (client: Client, isGerant: boolean) => {
...
const renderPrestations = (client: Client, isGerant: boolean) => {
...
const renderMandat = (client: Client, isGerant: boolean) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {client.statutMandat && <InfoField label="Statut" value={client.statutMandat} icon={<Clock size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />}
        {client.typeMandat && <InfoField label="Type" value={client.typeMandat} icon={<Compass size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />}
        ...
        {client.montantRemuneration && <InfoField label="Montant" value={(client as any).remunerationIsPercentage ? `${client.montantRemuneration}%` : `${client.montantRemuneration.toLocaleString()} ${client.devise || 'MAD'}`} icon={<DollarSign size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />}
      </div>
    </div>
  );
};

export const ClientCriteria = ({ client, isGerant = false }: { client: Client; isGerant?: boolean }) => {
        ...
        return (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <InfoField label="Situation actuelle" value={client.currentSituation || "Non spécifié"} icon={<User size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              <InfoField label="Urgence" value={client.urgency || "Non spécifié"} icon={<Clock size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              {client.moveInDate && (
              ...
              {client.classification && (
                <InfoField label="Classification" value={client.classification} icon={<Sliders size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              )}
              {client.categorie && (
                <InfoField label="Catégorie" value={client.categorie} icon={<Tag size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              )}
              {client.attributPrincipal && (
                <InfoField label="Attribut principal" value={client.attributPrincipal} icon={<Star size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              )}
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {client.criteres.map(c => (
                    <span key={c} className={isGerant ? "px-2 py-1 text-xs rounded-lg bg-[#E7D5D5] text-[#905D5D] border border-[#E0C6C6]" : "px-2 py-1 text-xs rounded-lg bg-accent/10 text-accent border border-accent/20"}>{c}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {client.vue && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoField label="Vue" value={client.vue} icon={<Eye size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
            ...
            {renderProximites(client, isGerant)}
            {renderPrestations(client, isGerant)}
            {renderMandat(client, isGerant)}
          </>
        );
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              {client.currentAddress && (
                <InfoField label="Adresse actuelle" value={client.currentAddress} icon={<MapPin size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              )}
              <InfoField label="Situation professionnelle" value={client.employmentStatus || "Non spécifié"} icon={<Briefcase size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              {client.moveInDate && (
                <InfoField label="Date souhaitée d'emménagement" value={new Date(client.moveInDate).toLocaleDateString('fr-FR')} icon={<Calendar size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
              )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <InfoField label="Type de bien recherché" value={client.propertyType || "Non spécifié"} icon={<Home size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
        <InfoField label={client.type === 'Voyageur' ? "Destination" : "Secteur géographique"} value={client.area || (client.secteur || "Non spécifié")} icon={<MapPin size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />} />
      ...
      {client.notes && (
        <div className="pt-2">
          <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
            <Tag size={16} className={isGerant ? 'text-[#905D5D]' : 'text-accent'} />
            Notes
          </h3>
