...
import {
  ...
  Mail, RefreshCw, Link, Award, TrendingUp
} from 'react-feather';
...
export const LocataireDetailTabs = ({ client: initialClient }: { client: Client }) => {
  ...
  const renderCriteres = () => {
    const hasData = client.propertyType || client.secteur || client.area || client.minSurface ||
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {client.prixMin !== undefined && (
            <InfoField label="Loyer max" value={`${client.prixMin.toLocaleString()} ${client.devise || 'MAD'}`} icon={<DollarSign size={16} className="text-accent" />} />
          )}
  ...
  const renderSolvabilite = () => {
    const hasData = client.employmentStatus || client.contribution || client.guarantor;
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="border-t border-border/30 pt-4">
          <h3 className="text-sm font-medium flex items-center gap-2 mb-3">
            <User size={16} className="text-premium" />
            Garant
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {client.guarantor && (
              <>
                <InfoField label="Nom du garant" value="Non renseigné" icon={<User size={16} className="text-premium" />} />
                <InfoField label="Revenus du garant" value="Non renseigné" icon={<TrendingUp size={16} className="text-premium" />} />
              </>
            )}
          </div>
        </div>
