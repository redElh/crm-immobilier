...
import { FileText, Clock, Trash2, ArrowRight, X, User, Briefcase, Users } from 'react-feather'
import { getDrafts, deleteDraft, type ContactDraft } from '../../../services/contactDraftStorage'
import { CompletionRing } from '../../ui/CompletionRing'

const CONTACT_TYPE_CONFIG: Record<string, { gradient: string; icon: React.ReactNode; label: string }> = {
  Particulier: { gradient: 'from-blue-500/20 to-blue-600/10', icon: <User size={15} />, label: 'Particulier' },
  Professionnel: { gradient: 'from-purple-500/20 to-purple-600/10', icon: <Briefcase size={15} />, label: 'Professionnel' },
  'Indivision / Succession': { gradient: 'from-orange-500/20 to-orange-600/10', icon: <Users size={15} />, label: 'Indivision' },
}

const DEFAULT_CONFIG = { gradient: 'from-gray-500/20 to-gray-600/10', icon: <User size={15} />, label: 'Contact' }
...
interface ContactDraftSectionProps {
  contactType?: string
  userId?: string
  onResume: (draft: ContactDraft) => void
  draftChange?: number
}

export function ContactDraftSection({ contactType, userId, onResume, draftChange }: ContactDraftSectionProps) {
  ...
  const loadDrafts = () => {
    ...
    setDrafts(getDrafts(userId, contactType))
  }

  useEffect(() => { loadDrafts() }, [contactType, userId, draftChange])
  ...
  const handleResume = (draft: ContactDraft) => {
    setOpen(false)
    onResume(draft)
  }
        ...
        {open && (
          ...
          >
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(100vh - 260px)' }}>
              {drafts.map((draft) => {
                const config = CONTACT_TYPE_CONFIG[draft.contactType] || DEFAULT_CONFIG
