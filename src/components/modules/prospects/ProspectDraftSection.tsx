...
import { FileText, Clock, Trash2, ArrowRight, X, ShoppingCart, Key, Tag, Compass } from 'react-feather'
import { getDrafts, deleteDraft, type ProspectDraft } from '../../../services/prospectDraftStorage'
import { CompletionRing } from '../../ui/CompletionRing'

const PROSPECT_TYPE_CONFIG: Record<string, { gradient: string; icon: React.ReactNode; label: string }> = {
  Acheter: { gradient: 'from-blue-500/20 to-blue-600/10', icon: <ShoppingCart size={15} />, label: 'Acheter' },
  Louer: { gradient: 'from-violet-500/20 to-violet-600/10', icon: <Key size={15} />, label: 'Louer' },
  Vendre: { gradient: 'from-emerald-500/20 to-emerald-600/10', icon: <Tag size={15} />, label: 'Vendre' },
  'Faire estimer': { gradient: 'from-amber-500/20 to-amber-600/10', icon: <Compass size={15} />, label: 'Estimer' },
}

const DEFAULT_CONFIG = { gradient: 'from-gray-500/20 to-gray-600/10', icon: <Compass size={15} />, label: 'Prospect' }
...
interface ProspectDraftSectionProps {
  userId?: string
  onResume: (draft: ProspectDraft) => void
  draftChange?: number
  onCountChange?: (count: number) => void
}

export function ProspectDraftSection({ userId, onResume, draftChange, onCountChange }: ProspectDraftSectionProps) {
  const [drafts, setDrafts] = useState<ProspectDraft[]>([])
  ...
  useEffect(() => { loadDrafts() }, [userId, draftChange])

  useEffect(() => { onCountChange?.(drafts.length) }, [drafts.length, onCountChange])
  ...
  const handleResume = (draft: ProspectDraft) => {
    setOpen(false)
    onResume(draft)
  }
  ...
  return (
    <>
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        layoutId="prospect-draft-bubble"
      >
        ...
        {open && (
          ...
          >
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(100vh - 260px)' }}>
              {drafts.map((draft) => {
                const prospectType = draft.data.type as string
                const config = PROSPECT_TYPE_CONFIG[prospectType] || DEFAULT_CONFIG
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-accent/10 text-accent flex-shrink-0">{config.label}</span>
                      </div>
