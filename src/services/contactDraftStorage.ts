...
const BASE_KEY = 'contactDrafts'

function storageKey(userId: string): string {
  return `${BASE_KEY}_${userId}`
}

function getAll(userId: string): ContactDraft[] {
  try {
...
function saveAll(userId: string, drafts: ContactDraft[]) {
  localStorage.setItem(storageKey(userId), JSON.stringify(drafts))
}

export function getDrafts(userId: string, type?: string): ContactDraft[] {
  const all = getAll(userId)
  return type ? all.filter(d => d.contactType === type) : all
}

export function getDraft(userId: string, id: string): ContactDraft | undefined {
  return getAll(userId).find(d => d.id === id)
}

export function saveDraft(userId: string, type: string, data: Record<string, any>, completion: number): ContactDraft {
  const all = getAll(userId)
