...
export async function up(pg) {
  const { rows: linkedClients } = await pg.query(`
  ...
  const { rows: allContacts } = await pg.query('SELECT id, mandats FROM contacts WHERE jsonb_array_length(mandats) > 0');
  for (const contact of allContacts) {
    ...
    for (const m of mandats) {
      ...
      const exists = await pg.query('SELECT 1 FROM owner_clients WHERE id = $1', [parseInt(m.clientId)]);
      if (exists.rows.length > 0) {
        cleaned.push(m);
      }
    }
    if (cleaned.length !== mandats.length) {
      await pg.query(
        'UPDATE contacts SET mandats = $1, updated_at = NOW() WHERE id = $2',
        [JSON.stringify(cleaned), contact.id]
      );
    }
  }
}
