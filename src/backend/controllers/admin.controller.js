...
import { generateToken, setTokenCookie } from '../config/auth.js';
...
export const getUsers = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, first_name, last_name, email, phone, role, is_active, created_at, updated_at FROM users ORDER BY created_at DESC'
    );
...
export const createUser = async (req, res) => {
  try {
    ...
    const newUser = await pool.query(
      ...
      [firstName, lastName, email, phone, hashedPassword, role || 'agent', is_active !== undefined ? is_active : true]
    );
...
export const updateUser = async (req, res) => {
  try {
    ...
    let query = 'UPDATE users SET first_name = $1, last_name = $2, email = $3, phone = $4, role = $5, is_active = $6, updated_at = CURRENT_TIMESTAMP';
    const params = [firstName, lastName, email, phone, role, is_active];
    let paramIndex = 7;
    ...
    query += ` WHERE id = $${paramIndex} RETURNING id, first_name, last_name, email, phone, role, is_active, updated_at`;
    params.push(id);
